...
class Wav2Vec:
     ...
     def inference(self, filename):
          ...
          beam_search_output = self.ngram_lm_model.decode(logits.cpu().detach().numpy(), beam_width=5)
...
if __name__ == "__main__":
    ...
    for i in range(0, 1):
        ...
        output_path=f"predict_wav2vec_{folder_name}"
        index=i
        if not os.path.exists(f"{output_path}"):
             os.makedirs(f"{output_path}")
        
        f = open(f"{output_path}/{index}.txt", "a", encoding="utf-8")
        for j in range(len(os.listdir(folder_name_audio))):
            audio_file = f"{folder_name_audio}/split_{j}.wav"
            ...
            if duration > 15:
                ...
                audio_filename, audio_rate = extract_audio(audio_file)
