...
def transciption(folder_name, output_path, index):
    ...
    f = open(f"predict_ngoc_huyen/{index}.txt", "a", encoding="utf-8")
    for i in range(len(os.listdir(folder_name))):
        segments, info = model.transcribe(f"{folder_name}/split_{i}.mp3", beam_size=5, without_timestamps=True )
...
for i in range(39, number_file + 1):
    print(f"In handle file with index: {i}")
    transciption(folder_name=f"split_Ngoc_Huyen/{i}", output_path="sub_Ngoc_Huyen", index=i)
